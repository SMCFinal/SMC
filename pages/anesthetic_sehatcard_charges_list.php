<?php
    ...
    $queryAnestheticCharges = mysqli_query($connect, "SELECT anesthetic_surgery_charges.*, discharge_patients.patient_doa, rooms.room_number,  discharge_patients.patient_operation, discharge_patients.patient_name, discharge_patients.patient_doop, surgeries.surgery_name, staff_members.name,  discharge_patients.organization FROM `anesthetic_surgery_charges` 
        ...
        WHERE anesthetic_surgery_charges.payment_status = '1' AND discharge_patients.organization LIKE '%Sehat%' AND anesthetic_surgery_charges.anesthetic_id = '$id'  GROUP BY anesthetic_surgery_charges.pat_id ORDER BY discharge_patients.patient_doa ASC LIMIT 100");
    ...
    if (isset($_POST['payAnestheticCharges'])) {
        ...
        $array_pat = $_POST['pat_id'];

        for ($j=0; $j < sizeof($array_pat); $j++) { 
            $pat = $array_pat[$j];

            $updatePaymentAnes = mysqli_query($connect, "UPDATE anesthetic_surgery_charges SET payment_status = '0', date_of_payment = '$currentDate' WHERE anesthetic_id = '$id' AND pat_id = '$pat'");
        }
...
<div class="page-content-wrapper ">
    <div class="container-fluid">
        <div class="row">
            <div class="col-sm-12">
                <h5 class="page-title">Anesthetic Charges (Sehat Card)</h5>
            </div>
        </div>
        <!-- end row -->
        <div class="row">
                                ...
                                <tbody>
                                    ...
                                    while ($rowSurgery = mysqli_fetch_assoc($queryAnestheticCharges)) {
                                        ...
                                        echo '
                                            ...
                                            <input type="hidden" name="pat_id[]"  value="'.$rowSurgery['pat_id'].'">
                                        </tr>
                                        ';
                                    }
